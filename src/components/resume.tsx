export const Resume = () => {
  return (
    <section id="resume">
      <h1>Resume</h1>
      <div className="resumeGrid">
        <div className="resumeCardContainer">
          <h2>Professional Summary</h2>
          <p className="professionalSummaryStyling">
            After a decade in the offshore oil gas industry working as a
            production technician, I stepped away from my career to be home with
            my family and support my Dad.
            <br />
            <br />
            While caring for my Dad and navigating the many challenges of
            dementia, I found a new passion in tech and made the decision to
            swap steel caps for a keyboard one day. After my Dad moved into a
            high-care memory support unit in 2021, I found an amazing company
            who were happy for me to begin my IT journey with them part-time so
            I could still support my Dad until his passing in 2022.
            <br />
            <br />
            In 2024 I discovered my passion for software engineering after
            enrolling in a Certificate IV in IT (Programming). Having recently
            completed my studies at TAFE, I am looking forward to begining my
            studies at Murdoch University where I have been accepted into a
            Bachelor of Information Technology. I'll be completing my degree
            part-time, so I am really excited about having the opportunity to
            find a great company to work with in a full-time capacity.
          </p>
        </div>
        <div className="resumeCardContainer">
          <h2>Work Experience</h2>
          <div className="workExperienceCardContainer">
            <div>
              <h3>Software Engineering Student</h3>
              <h4>TAFE & University</h4>
            </div>
            <h6>
              <em>Jul 2024 - Current</em>
            </h6>
            <p>
              Currently gaining hands-on experience in software engineering
              principles through a combination of practical projects and
              academic coursework.
            </p>
            <hr />
            <div>
              <h3>Associate Systems Engineer</h3>
              <h4>Kinetic IT</h4>
              <h5>(Full-time &#183; Hybrid)</h5>
            </div>
            <h6>
              <em>Aug 2022 - Jan 2023</em>
            </h6>
            <p>
              As a junior engineer on the SOE team, I was responsible for patch
              management for Windows and 3rd party patches on over 7000 devices
              each month.
            </p>
            <hr />
            <div>
              <h3>Response Team Member</h3>
              <h4>TechBrain</h4>
              <h5>(Part-time &#183; Hybrid)</h5>
            </div>
            <h6>
              <em>Nov 2021 - Sep 2024</em>
            </h6>
            <p>
              Delivered technical support in a service desk role,
              troubleshooting hardware, software and network issues for end
              users.
            </p>
            <hr />
            <div>
              <h3>Caregiving</h3>
              <h4>Career Break</h4>
              <h5>(Full-time until 2021, then part-time until 2022)</h5>
            </div>
            <h6>
              <em>Apr 2018 - Dec 2022</em>
            </h6>
            <p>
              Supported my Dad in living his best life throughout his battle
              against Dementia.
            </p>
            <hr />
            <div>
              <h3>Offshore Production Technician</h3>
              <h4>Woodside Energy & Inpex</h4>
              <h5>(Full-time)</h5>
            </div>
            <h6>
              <em> Aug 2008 - Mar 2018</em>
            </h6>
            <p>
              As an offshore production technician I was responsible for the
              safe and efficient operation of process systems in high-pressure,
              high-risk environments.
            </p>
          </div>
        </div>

        <div className="resumeCardContainer">
          <h2>Education</h2>
          <div className="educationCardContainer">
            <div>
              <h3>Bachelor of Information Technology</h3>
              <h4>Murdoch University</h4>
            </div>
            <div>
              <h5>
                <em>Jul 2025 - Jul 2031</em>
              </h5>
            </div>
            <div>
              <p>Double major: Computer Science and second major TBC.</p>
            </div>
            <hr />
            <div>
              <h3>Certificate IV in IT (Programming)</h3>
              <h4>South Metropolitan TAFE</h4>
            </div>
            <div>
              <h5>
                <em>Jul 2024 - Jun 2025</em>
              </h5>
            </div>
            <div>
              <p>
                Focus: Object-oriented languages skills, object-oriented design
                and testing, artificial intelligence, emerging technologies, web
                and application development, database management and development
                methodoligies.
              </p>
            </div>
          </div>
        </div>

        <div className="resumeCardContainer">
          <h2>Technical Skills</h2>
          <div className="mobileGridTemplate technicalCardContainer">
            <div className="mobileGridCardContainer">
              <h3>Languages</h3>
              <ul className="smallCardContainer">
                <li>C#</li>
                <li>Java</li>
                <li>CSS</li>
                <li>HTML</li>
                <li>TypeScript</li>
              </ul>
            </div>
            <hr />
            <div className="mobileGridCardContainer">
              <h3>Software Development</h3>
              <ul className="smallCardContainer">
                <li>Version Control</li>
                <li>Unit Testing</li>
                <li>Agile Methodology</li>
              </ul>
            </div>
            <hr />
            <div className="mobileGridCardContainer">
              <h3>Frameworks | Libraries</h3>
              <ul className="smallCardContainer">
                <li>JUnit</li>
                <li>NUnit</li>
                <li>React</li>
              </ul>
            </div>
            <hr />
            <div className="mobileGridCardContainer">
              <h3>Tools & IDEs</h3>
              <ul className="smallCardContainer">
                <li>Vite</li>
                <li>VS Code</li>
                <li>Visual Studio</li>
                <li>Azure DevOps</li>
                <li>Git | GitHub</li>
                <li>NetBeans</li>
              </ul>
            </div>
            <hr />
            <div className="mobileGridCardContainer">
              <h3>Databases</h3>
              <ul className="smallCardContainer">
                <li>Microsoft SQL Server 2022</li>
              </ul>
            </div>
            <hr />
            <div className="mobileGridCardContainer">
              <h3>Soft Skills</h3>
              <ul className="smallCardContainer">
                <li>Problem-solving</li>
                <li>Adaptability</li>
                <li>Communication</li>
                <li>Teamwork</li>
                <li>Critical Thinking</li>
              </ul>
            </div>
            <hr />
            <div className="mobileGridCardContainer">
              <h3>Cloud</h3>
              <ul className="smallCardContainer">
                <li>Microsoft 365</li>
                <li>Familiar with AWS & Azure</li>
              </ul>
            </div>
            <hr />
            <div className="mobileGridCardContainer">
              <h3>Operating Systems</h3>
              <ul className="smallCardContainer">
                <li>Windows</li>
              </ul>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};
